...
prepare_phylogenetic_tree <- function(tree) {
  # Preprocess phylogenetic tree to ensure it's suitable for ancestral state reconstruction
  ...
  # Create a copy of the tree to avoid modifying the original object
  tree_copy <- tree
  has_issues <- FALSE
  
  # Check and fix root node issues
  if(!is.rooted(tree_copy)) {
    message("Tree is not rooted. Attempting to root at midpoint...")
    has_issues <- TRUE
    
    if(requireNamespace("phytools", quietly = TRUE)) {
      # Try using midpoint rooting (better method)
      tree_copy <- tryCatch({
  ...
  # Ensure all branch lengths are valid positive numbers
  if(is.null(tree_copy$edge.length)) {
    ...
    # Check for zero or near-zero branch lengths
    ...
    if(length(zero_branches) > 0) {
      ...
      # Replace zero or near-zero branch lengths with small positive value (0.0001)
      tree_copy$edge.length[zero_branches] <- 0.0001
      
      # Calculate total length after replacement to confirm fix
      total_length <- sum(tree_copy$edge.length)
      message(paste("After fixing, total tree length:", round(total_length, 4)))
    }
  }
  
  # Ensure node numbering is sequential and valid
  ...
  # Check branch length distribution to detect anomalous branches
...
perform_ml_reconstruction <- function(tree, count_vector) {
  # Specialized handler for ML ancestral state reconstruction with enhanced robustness
  ...
  if(length(count_vector) != length(tree$tip.label)) {
    ...
    # Ensure count_vector matches tree tip.labels
  ...
  if(na_count > 0) {
    ...
    # No imputation here, let ace function handle it
  }
  
  # Try ML method
  tryCatch({
    # Standard ace function call with explicit parameter setting
...
perform_bayesian_reconstruction <- function(tree, count_vector) {
  # Specialized handler for Bayesian ancestral state reconstruction with enhanced robustness
  ...
  # Simplified MCMC parameters for better success rate
  ngen <- 30000        # Reduced generations to avoid long runtime
  samplefreq <- 100    # Sampling frequency
  burnin <- 0.2        # burn-in proportion
  control_params <- list(
    sig2 = var(count_vector, na.rm = TRUE), # Use data variance as initial value
...
# ==============================
# Chromosome Count Constraints Functions
# ==============================

apply_biological_constraints <- function(node_counts, min_count = 3, max_count = 100) {
  # Apply basic biological constraints to all nodes
  message("Applying global biological constraints to chromosome counts...")
  
  # Statistics on original counts
  invalid_counts <- sum(node_counts <= 0 | node_counts > max_count, na.rm = TRUE)
  if(invalid_counts > 0) {
    message(sprintf("Found %d biologically implausible chromosome counts before correction", invalid_counts))
  }
  
  # Apply minimum value constraint (avoid zero and negative values)
  node_counts[node_counts < min_count] <- min_count
  
  # Apply maximum value constraint (avoid unreasonably high values)
  node_counts[node_counts > max_count] <- max_count
  
  message("Global biological constraints applied")
  
  return(node_counts)
}

apply_node_constraints <- function(node_counts, node_constraints) {
  # Apply user-specified node-specific constraints
  if(is.null(node_constraints) || length(node_constraints) == 0) {
    return(node_counts) # Return directly if no constraints
  }
  
  message("Applying user-defined node constraints...")
  
  count_before <- node_counts
  
  # Iterate through each constraint
  for(node_id in names(node_constraints)) {
    if(node_id %in% names(node_counts)) {
      constraint_value <- node_constraints[[node_id]]
      original_value <- node_counts[node_id]
      
      # Apply constraint
      node_counts[node_id] <- constraint_value
      
      message(sprintf(" - Node %s: constraint applied (%.1f → %.1f)", 
                    node_id, original_value, constraint_value))
    } else {
      message(sprintf(" - Node %s: constraint specified but node not found in results", node_id))
    }
  }
...
find_root_node <- function(tree) {
  # Determine the root node ID of the tree
  n_tips <- length(tree$tip.label)
  
  if(is.rooted(tree)) {
    # For rooted trees, root node is typically the first internal node
    root_id <- n_tips + 1
    message(sprintf("Found root node at ID: %d", root_id))
    return(root_id)
  } else {
    # For unrooted trees, we need to infer the root node
    message("Tree is not rooted, attempting to infer root node")
    
    # Look for most likely root node (typically a node with degree 2)
    edge_counts <- table(c(tree$edge[,1], tree$edge[,2]))
    potential_roots <- as.integer(names(edge_counts[edge_counts == 2]))
    
    # Filter to internal nodes
    potential_roots <- potential_roots[potential_roots > n_tips]
    
    if(length(potential_roots) > 0) {
      root_id <- min(potential_roots)
      message(sprintf("Inferred root node at ID: %d", root_id))
      return(root_id)
    } else {
      # If root cannot be determined, use first internal node
      root_id <- n_tips + 1
      message(sprintf("Could not infer root, using first internal node: %d", root_id))
      return(root_id)
    }
  }
}

validate_chromosome_counts <- function(node_counts, tree, chr_counts) {
  # Validate reasonableness of ancestral chromosome counts
  message("Validating reconstructed chromosome counts...")
  
  # Find root node
  root_id <- find_root_node(tree)
  
  # Get tip nodes and internal nodes distribution
  n_tips <- length(tree$tip.label)
  tip_counts <- node_counts[1:n_tips]
  internal_counts <- node_counts[(n_tips+1):length(node_counts)]
  
  # Calculate and display statistics
  message(sprintf("Root node (ID: %d) chromosome count: %.1f", 
                root_id, node_counts[as.character(root_id)]))
  
  message(sprintf("Tip nodes count range: %.1f to %.1f (median: %.1f)", 
                min(tip_counts, na.rm=TRUE), 
                max(tip_counts, na.rm=TRUE), 
                median(tip_counts, na.rm=TRUE)))
  
  message(sprintf("Internal nodes count range: %.1f to %.1f (median: %.1f)", 
                min(internal_counts, na.rm=TRUE), 
                max(internal_counts, na.rm=TRUE), 
                median(internal_counts, na.rm=TRUE)))
  
  # Check for values below 1 (biologically impossible)
  implausible_low <- sum(node_counts < 1, na.rm=TRUE)
  if(implausible_low > 0) {
    message(sprintf("WARNING: %d nodes have implausible low counts (<1)", implausible_low))
  }
  
  # Check for unreasonably high values
  implausible_high <- sum(node_counts > 100, na.rm=TRUE)
  if(implausible_high > 0) {
    message(sprintf("WARNING: %d nodes have potentially implausible high counts (>100)", implausible_high))
  }
  
  # Compare tip nodes with actual observed values
  tip_diffs <- numeric(n_tips)
  names(tip_diffs) <- tree$tip.label
  
  for(tip in names(chr_counts)) {
    if(tip %in% tree$tip.label) {
      tip_id <- which(tree$tip.label == tip)
      if(length(tip_id) > 0) {
        reconstructed <- node_counts[as.character(tip_id)]
        observed <- chr_counts[tip]
        tip_diffs[tip] <- reconstructed - observed
      }
    }
  }
  
  # Calculate difference between tip node reconstruction and actual values
  tip_diff_mean <- mean(abs(tip_diffs), na.rm=TRUE)
  message(sprintf("Mean absolute difference between reconstructed and observed tip values: %.2f", tip_diff_mean))
  
  return(list(
    root_id = root_id,
    root_count = node_counts[as.character(root_id)],
    tip_counts_stats = summary(tip_counts),
    internal_counts_stats = summary(internal_counts),
    tip_differences = tip_diffs,
    mean_tip_difference = tip_diff_mean,
    implausible_low_count = implausible_low,
    implausible_high_count = implausible_high
  ))
}
...
reconstruct_ancestral_chromosome_counts <- function(tree, chr_counts, method = "ML", 
  ...
  # Ancestral chromosome count reconstruction, integrating various methods and applying constraints
  message(paste("Reconstructing ancestral chromosome counts using", method, "method..."))
  
  # Preprocess phylogenetic tree to ensure it meets analysis requirements
  ...
  # Phylogenetic imputation for missing values
  if(any(is.na(count_vector))) {
    ...
    for(sp in missing_tips) {
      ...
      if(length(na.omit(related_counts)) > 0) {
        ...
        # If no related species data, use global median
  ...
  # Ancestral state reconstruction method selection
  ...
  if(method == "bayesian") {
    ...
    if(!is.null(bayes_result)) {
      ...
      # Save original inference results before applying constraints
      unconstrained_counts <- node_counts
      
      # Apply biological constraints
      node_counts <- apply_biological_constraints(node_counts, min_count, max_count)
      
      # Apply user-defined node constraints
      if(!is.null(node_constraints)) {
        node_counts <- apply_node_constraints(node_counts, node_constraints)
      }
      
      # Validate results
      validation <- validate_chromosome_counts(node_counts, tree, chr_counts)
      
      result <- list(
        node_counts = node_counts,
        unconstrained_counts = unconstrained_counts,
  ...
  # If all statistical methods fail, use topology-based interpolation
  if(!success) {
    message("All statistical methods failed, using topology-based interpolation")
    
    # Fill from tip nodes upwards
    for(node_id in (n_tips+1):(n_tips+n_nodes)) {
      # Get all direct child nodes of this node
      child_nodes <- tree$edge[tree$edge[,1] == node_id, 2]
      
      # Get chromosome counts of child nodes
      child_counts <- node_counts[as.character(child_nodes)]
      
      # Use weighted median of child nodes (weighted by branch length)
      child_weights <- rep(1, length(child_counts))
      
      # If branch length information is available, use its inverse as weights
    ...
    # Create basic ace_result for compatibility
    ace_result <- list(
...
combine_event_analyses <- function(bottom_up_events, top_down_events) {
  # Combine bottom-up and top-down event analysis results, ensuring column structure compatibility
  message("Combining bottom-up and top-down event analyses with enhanced integration...")
  
  # First check if we have valid data frames
  ...
  # If both are empty, return empty data frame
  ...
  # Create a function to standardize data frames
  standardize_df <- function(df, ref_df = NULL) {
    # Standardize columns of a single data frame
    if(nrow(df) == 0) return(df)
    
    # If no reference data frame provided, add some basic columns
    if(is.null(ref_df)) {
    ...
    # All possible columns (union of columns from both data frames)
    all_columns <- unique(c(names(df), names(ref_df)))
    
    # Align columns: add missing columns and apply correct types
    for(col in all_columns) {
      if(!(col %in% names(df))) {
        # If column is missing in df, add a column with appropriate NA values
        if(col %in% names(ref_df)) {
          # Determine type based on ref_df
          ...
          # If missing in ref_df as well, infer based on column name
  ...
  # Handle single data frame cases
  if(have_bottom_up && !have_top_down) {
  ...
  # If both data frames exist, first add origin markers to each
  bottom_up_events$analysis_orig <- "bottom_up"
  top_down_events$analysis_orig <- "top_down"
  ...
  # Standardize by mutual reference
  ...
  # Recheck once more to ensure full alignment
  ...
  # Output column information for debugging
  ...
  # Use rbind to combine standardized data frames
  if(ncol(bu_standardized) != ncol(td_standardized)) {
    # Final safety measure - use more reliable data.table method
    if(requireNamespace("data.table", quietly = TRUE)) {
    ...
    # Standard rbind should work safely
    all_events <- rbind(bu_standardized, td_standardized)
  }
  
  # Initialize corroboration and convergence columns
  ...
  # Detect convergent evolution patterns
  if(requireNamespace("stats", quietly = TRUE) && nrow(all_events) >= 5) {
    tryCatch({
      # Analyze each major event type
      for(evt_type in c("fusion", "fission")) {
        ...
        if(nrow(type_events) >= 3) {
          # Cluster events by count difference
          ...
          for(cluster_id in unique(clusters)) {
            ...
            # If multiple events have similar changes in different species
            if(nrow(cluster_events) >= 2) {
              species_set <- unique(cluster_events$child_label)
              
              if(length(species_set) >= 2) {
                # Mark as potential convergent evolution
                group_id <- paste0("convergent_", evt_type, "_", cluster_id)
                
                # Update convergent_evolution and convergent_group columns
  ...
  # Summarize convergent evolution findings
...
detect_shared_chromosome_events <- function(integrated_data, conserved_groups, anc_method = "ML", 
                                          genomic_data = NULL, min_chr_count = 1, max_chr_count = 100,
                                          node_constraints = NULL) {
  ...
  # Reconstruct ancestral chromosome counts using specified method and constraints
  ancestral_counts <- reconstruct_ancestral_chromosome_counts(
    tree, chr_counts, method = anc_method,
    min_count = min_chr_count, max_count = max_chr_count,
...
main <- function(args) {
  # Check arguments
  if(length(args) < 3) {
    cat("Usage: Rscript ancestral_chromosomes_phase3.v5.R integrated_data_file conserved_groups_file output_dir [anc_method] [genomic_data_file] [min_chr_count] [max_chr_count] [node_constraints_file]\n")
    cat("Example: Rscript ancestral_chromosomes_phase3.v5.R results/integrated_data.RData results/phase2/conserved_chromosome_groups.RData results/phase3/ ML\n")
    cat("Options:\n")
    ...
    cat("  min_chr_count: Minimum plausible chromosome count [default: 1]\n")
    cat("  max_chr_count: Maximum plausible chromosome count [default: 100]\n")
    cat("  node_constraints_file: Optional RData file with specific node constraints\n")
    return(1)
  }
  ...
  min_chr_count <- if(length(args) >= 6) as.numeric(args[6]) else 1
  max_chr_count <- if(length(args) >= 7) as.numeric(args[7]) else 100
  
  # Handle node constraints if provided
  node_constraints <- NULL
  if(length(args) >= 8) {
    node_constraints_file <- args[8]
    if(file.exists(node_constraints_file)) {
      node_constraints <- tryCatch({
        readRDS(node_constraints_file)
      }, error = function(e) {
        message("Warning: Could not load node constraints file, proceeding without constraints")
        NULL
      })
    }
  }
  ...
  event_result <- detect_shared_chromosome_events(
    integrated_data, 
    conserved_groups, 
    anc_method, 
    genomic_data, 
