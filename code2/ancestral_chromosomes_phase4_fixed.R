...
load_previous_phases <- function(phase1_file, phase2_file, phase3_file) {
  ...
  # 验证事件数据结构
  if(!is.null(event_results$events)) {
    message("DEBUG: events data.frame has columns: ", paste(names(event_results$events), collapse=", "))
    message("DEBUG: events contains ", nrow(event_results$events), " rows")
    message("DEBUG: parent_node NA count: ", sum(is.na(event_results$events$parent_node)))
    message("DEBUG: child_node NA count: ", sum(is.na(event_results$events$child_node)))
  } else {
    message("DEBUG: event_results does not contain 'events' element")
  }
...
prepare_node_reconstruction <- function(tree, event_results) {
  ...
  node_contexts <- lapply(internal_nodes, function(node_id) {
    ...
    # Get events where this node is involved - 增加防御性检查
    node_events <- data.frame()
    if(!is.null(event_results$events) && is.data.frame(event_results$events)) {
      # 安全索引确保不会返回空结构 - 增加NA值检查
      node_events <- event_results$events[
        (event_results$events$parent_node == node_id | 
           event_results$events$child_node == node_id) & 
          !is.na(event_results$events$parent_node) & 
          !is.na(event_results$events$child_node), 
      ]
    ...
    shared_events <- data.frame()
    if(!is.null(event_results$shared_events) && is.data.frame(event_results$shared_events)) {
      shared_events <- event_results$shared_events[
        event_results$shared_events$clade_node_id == node_id & 
          !is.na(event_results$shared_events$clade_node_id), 
      ]
...
detect_communities <- function(g) {
  ...
  # 从配置中读取社区检测方法，或使用默认方法
  methods <- if("community_detection_methods" %in% names(config)) {
    config$community_detection_methods
  } else {
    c("louvain", "fast_greedy", "label_prop", "walktrap")
  }
  ...
  # 获取小社区阈值
  small_threshold <- if("small_community_threshold" %in% names(config)) {
    config$small_community_threshold
  } else {
    3  # 默认值
  }
  
  # Filter small communities
  small_communities <- which(sapply(communities, nrow) < small_threshold)
...
get_community_confidence <- function(comm_markers) {
  ...
  # 获取权重或使用默认值
  conservation_weight <- if("conservation_weight" %in% names(config)) config$conservation_weight else 0.6
  coverage_weight <- if("species_coverage_weight" %in% names(config)) config$species_coverage_weight else 0.4
  
  # Combine metrics using configurable weights
  confidence <- (conservation_weight * group_consistency) + 
              (coverage_weight * min(1, n_species / 5))
...
reconstruct_by_event_reversal <- function(context, conserved_groups, event_results) {
  ...
  message("DEBUG: context node_id =", context$node_id, ", node_label =", context$node_label)
  ...
  # 添加更多调试信息
  message("DEBUG: relevant_events has", nrow(relevant_events), "rows")
  if(nrow(relevant_events) > 0) {
    message("DEBUG: relevant_events columns: ", paste(names(relevant_events), collapse=", "))
  }
  ...
  # 添加调试信息
  message("DEBUG: Found", nrow(fusion_events), "fusion events")
  message("DEBUG: fusion_events has NA in parent_node: ", 
         ifelse(nrow(fusion_events) > 0, sum(is.na(fusion_events$parent_node)), "N/A"))
  message("DEBUG: fusion_events has NA in child_node: ",
         ifelse(nrow(fusion_events) > 0, sum(is.na(fusion_events$child_node)), "N/A"))
  ...
  if(nrow(fusion_events) > 0) {
    ...
    for(i in 1:nrow(fusion_events)) {
      ...
      # 添加调试信息
      message(paste0("DEBUG: Processing fusion event ", i, 
                    ", parent_node=", event$parent_node, 
                    ", child_node=", event$child_node,
                    ", context node_id=", context$node_id))
      
      # 增加NA值检查
      if(!is.na(event$parent_node) && event$parent_node == context$node_id) {
  ...
  # 添加调试信息
  message("DEBUG: Found", nrow(fission_events), "fission events")
  message("DEBUG: fission_events has NA in parent_node: ", 
         ifelse(nrow(fission_events) > 0, sum(is.na(fission_events$parent_node)), "N/A"))
  ...
  if(nrow(fission_events) > 0) {
    ...
    for(i in 1:nrow(fission_events)) {
      ...
      # 添加调试信息
      message(paste0("DEBUG: Processing fission event ", i, 
                    ", parent_node=", event$parent_node, 
                    ", context node_id=", context$node_id))
  ...
  if("allow_wgd_events" %in% names(config) && config$allow_wgd_events) {
    message("DEBUG: WGD event detection enabled")
    
    # 获取所有子节点
    child_nodes <- context$descendants
    message("DEBUG: Found", length(child_nodes), "descendant nodes to check for WGD events")
    ...
    for(child_node in child_nodes) {
      ...
      message("DEBUG: Child node", child_node, "has chromosome count", child_count)
      message("DEBUG: Current node", context$node_id, "has chromosome count", context$chr_count)
      
      # 从配置中获取比例阈值或使用默认值
      max_chromosome_ratio <- if("max_chromosome_ratio" %in% names(config)) {
        config$max_chromosome_ratio
      } else {
        2.5  # 默认值
      }
      ...
      if(!is.na(child_count) && !is.na(context$chr_count) && 
         (child_count / context$chr_count) >= max_chromosome_ratio) {
    ...
    message("DEBUG: WGD event detection disabled or not configured")
  }
...
reconstruct_bottom_up <- function(node_contexts, linkage_markers, tree, conserved_groups) {
  ...
  for(idx in node_contexts$bottom_up_order) {
    ...
    taxon_rates <- if("taxon_specific_rates" %in% names(config)) {
      config$taxon_specific_rates
    } else {
      list("Mammalia" = 0.8, "Aves" = 0.5, "Teleostei" = 1.2, "default" = 1.0)
    }
    
    rate_factor <- taxon_rates[[result[[as.character(node_id)]]$taxon]]
...
reconstruct_by_parsimony <- function(markers, context, conserved_groups) {
  ...
  # 从配置中获取覆盖率阈值或使用默认值
  parsimony_threshold <- if("parsimony_coverage_threshold" %in% names(config)) {
    config$parsimony_coverage_threshold
  } else {
    0.5  # 默认值
  }
  ...
  for(i in seq_along(conserved_groups$conserved_groups)) {
    ...
    if(coverage >= parsimony_threshold) {
...
integrate_child_nodes <- function(current_results, context, conserved_groups) {
  ...
  # 从配置中获取置信度修正系数或使用默认值
  fusion_penalty <- if("fusion_confidence_penalty" %in% names(config)) {
    config$fusion_confidence_penalty
  } else {
    0.9  # 默认值
  }
  
  fission_penalty <- if("fission_confidence_penalty" %in% names(config)) {
    config$fission_confidence_penalty
  } else {
    0.85  # 默认值
  }
  ...
  for(child_id in processed_children) {
    ...
    if(nrow(events) > 0) {
      ...
      for(i in 1:nrow(events)) {
        ...
        # 增加NA值检查
        if(is.na(event$parent_node) || is.na(event$child_node)) {
          message("DEBUG: Skipping event with NA parent_node or child_node")
          next
        }
          ...
          if(!is.na(event$group) && event$group %in% names(child_groups)) {
            ...
            for(j in 1:2) {
              result[[paste0("fusion_parent_", event$group, "_", j)]] <- list(
                ...
                confidence = orig_group$confidence * fission_penalty,
          ...
          if(!is.na(event$group)) {
            ...
            if(length(split_groups) >= 2) {
              ...
              result[[paste0("fission_parent_", event$group)]] <- list(
                ...
                confidence = merged_confidence * fusion_penalty,
...
combine_reconstruction_methods <- function(parsimony_result, network_result, event_result, child_integration, expected_count) {
  ...
  for(group in unique_groups) {
    ...
    if(length(instances) == 1) {
      ...
      # 从配置中获取信度提升系数或使用默认值
      common_boost <- if("common_group_boost" %in% names(config)) {
        config$common_group_boost
      } else {
        1.2  # 默认值
      }
      ...
      if(!is.na(event$group)) {
        ...
        if(length(affected_idx) >= 2) {
          ...
          result[[merged_name]] <- list(
            ...
            confidence = mean(sapply(result[affected_idx[1:2]], function(g) g$confidence)) * fusion_penalty,
            method = "fusion_event",
        ...
        if(length(affected_idx) > 0) {
          ...
          for(j in 1:2) {
            ...
            result[[split_name]] <- list(
              ...
              confidence = result[[affected_idx[1]]]$confidence * fission_penalty,
...
integrate_node_approaches <- function(bottom_up_groups, top_down_groups, context) {
  ...
  unique_penalty <- if("unique_group_penalty" %in% names(config)) {
    config$unique_group_penalty
  } else {
    0.9  # 默认值
  }
  ...
  for(group in common_groups) {
    ...
    int_conf <- (bu_conf + td_conf) / 2 * common_boost
  ...
  for(group in unique_bu_groups) {
    ...
    result[[name]]$confidence <- result[[name]]$confidence * unique_penalty
...
generate_linkage_heatmap <- function(ancestral_linkage, integrated_data, conserved_groups, output_file) {
  ...
  if(!requireNamespace("pheatmap", quietly = TRUE)) {
    message("pheatmap package is required for generating heatmaps. Skipping heatmap generation.")
    return(NULL)
  }
...
reconstruct_ancestral_linkage_groups <- function(phase1_file, phase2_file, phase3_file, output_dir, custom_config = NULL) {
  ...
  # Only try to generate heatmap if pheatmap is available
  heatmap_matrix <- tryCatch({
    if(requireNamespace("pheatmap", quietly = TRUE)) {
      generate_linkage_heatmap(
        integrated_linkage, 
        prev_data$integrated_data, 
        prev_data$conserved_groups, 
      ...
      message("pheatmap package not available. Skipping heatmap generation.")
      NULL
    }
  }, error = function(e) {
    message(paste("Warning: Could not create linkage heatmap:", e$message))
    NULL
  })
...
if(!interactive()) {
  ...
  if(length(args) < 3) {
    cat("Usage: Rscript ancestral_chromosomes_phase4_fixed.R <integrated_data_file> <conserved_groups_file> <chromosome_events_file> [output_dir] [config_file]\n")
    cat("Example: Rscript ancestral_chromosomes_phase4_fixed.R results/integrated_data.RData results/phase2/conserved_chromosome_groups.RData results/phase3/chromosome_events.RData results/phase4\n")
    quit(status = 1)
  }
