...
def create_protein_msa_supergene(all_gene_results, output_file_base, 
    ...
    try:
        ...
        # 更健壮的路径构建
        base_dir = os.path.dirname(os.path.dirname(output_file_base))
        protein_supergene_dir = os.path.join(base_dir, "protein_supergene")
        os.makedirs(protein_supergene_dir, exist_ok=True)
        
        # 基于原始文件名构建新的输出路径
        base_filename = os.path.basename(output_file_base).replace('_4d', '_protein')
        if not base_filename.endswith('.fasta'):
            base_filename += '.fasta'
        output_file = os.path.join(protein_supergene_dir, base_filename)
        
        logger.info(f"蛋白质MSA超基因将输出到: {output_file}")
        ...
        # 保存单独的蛋白质序列到protein目录
        save_protein_sequences(protein_gene_results, base_dir)
        
        # 对蛋白质序列进行比对，生成比对结果到protein_alignments目录
        if aligner_params:
            align_protein_sequences(protein_gene_results, base_dir, aligner_params)
        
        # 合并蛋白质序列，创建超基因保存到protein_supergene目录
...
def create_full_cds_supergene(all_gene_results, output_file_base, 
    ...
    try:
        ...
        # 更健壮的路径构建
        base_dir = os.path.dirname(os.path.dirname(output_file_base))
        full_cds_dir = os.path.join(base_dir, "full_cds")
        os.makedirs(full_cds_dir, exist_ok=True)
        
        # 基于原始文件名构建新的输出路径
        base_filename = os.path.basename(output_file_base).replace('_4d', '_full_cds')
        if not base_filename.endswith('.fasta'):
            base_filename += '.fasta'
        output_file = os.path.join(full_cds_dir, base_filename)
        
        logger.info(f"完整CDS超基因将输出到: {output_file}")
...
def save_protein_sequences(protein_gene_results, output_dir):
    """
    保存单个基因的蛋白质序列到protein目录
    
    Args:
        protein_gene_results: 包含蛋白质序列数据的基因结果列表
        output_dir: 输出目录根路径
    ...
    saved_count = 0
    for gene_result in protein_gene_results:
        gene_name = gene_result['name']
        protein_file = os.path.join(protein_dir, f"{gene_name}_protein.fasta")
        
        try:
            with open(protein_file, 'w') as f:
                for species, seq in gene_result['sequences'].items():
                    f.write(f">{species}\n")
                    # 每行60个字符，提高可读性
                    for i in range(0, len(seq), 60):
                        f.write(f"{seq[i:i+60]}\n")
            saved_count += 1
            logger.debug(f"保存蛋白质序列至 {protein_file}")
        except Exception as e:
            logger.error(f"保存蛋白质序列 {gene_name} 时出错: {str(e)}")
    
    logger.info(f"成功保存 {saved_count} 个基因的蛋白质序列到 {protein_dir}")
    return saved_count

def align_protein_sequences(protein_gene_results, output_dir, aligner_params):
    """
    对蛋白质序列进行比对并保存结果到protein_alignments目录
    ...
    """
    protein_dir = os.path.join(output_dir, "protein")
    protein_alignments_dir = os.path.join(output_dir, "protein_alignments")
    os.makedirs(protein_alignments_dir, exist_ok=True)
    
    # 创建临时目录用于比对处理
    temp_dir = os.path.join(output_dir, "temp", "protein_alignments")
    os.makedirs(temp_dir, exist_ok=True)
    ...
    with ThreadPoolExecutor(max_workers=min(4, os.cpu_count() or 2)) as executor:
        ...
        for gene_result in protein_gene_results:
            gene_name = gene_result['name']
            input_file = os.path.join(temp_dir, f"{gene_name}_protein_input.fasta")
            output_file = os.path.join(protein_alignments_dir, f"{gene_name}_protein_aligned.fasta")
            ...
            # 提交比对任务
            modified_params = aligner_params.copy()
            modified_params['codon_aware'] = False  # 蛋白质序列不需要密码子感知
        ...
        for future in futures:
            ...
            try:
                ...
                if success:
                    aligned_count += 1
                    logger.info(f"成功比对蛋白质序列 {gene_name}")
                else:
                    logger.warning(f"比对蛋白质序列 {gene_name} 失败")
            except Exception as e:
                logger.error(f"处理蛋白质序列比对 {gene_name} 时出错: {str(e)}")
    ...
    try:
        ...
        logger.warning(f"清理蛋白质比对临时文件时出错: {str(e)}")
    
    logger.info(f"成功比对 {aligned_count} 个基因的蛋白质序列到 {protein_alignments_dir}")
...
def main():
    ...
    try:
        ...
        # 创建必要的子目录
        subdirs = [
            "alignments", "4d_sites", "full_cds", 
            "protein", "protein_alignments", "protein_supergene", 
            "stats", "temp"
        ]
